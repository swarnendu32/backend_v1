...
> = {
  ...
  /**timestamp when request started processing */
  timestamp: number;
...
export type PagePayload<T extends {}> = {
  /**true if there are any more pages left */
  hasMorePages: boolean;
  /**list of data in the page */
  data: T[];
};
...
export type ResponseBodyParams<
  ...
  V extends {} = never
> = {
  /**payload assoiciated with the response if successfull */
  payload?: P;
  /**metadata associated with the response */
  meta: ResponseMetaData<T, U, V>;
  /**error associated with the response */
  error?: AppError;
};

/**query params to fetch an account */
export type AccountRequestQueryParams = {
  username: string;
};

/**query params to fetch a memory */
export type MemoryRequestQueryParams = {
  memoryId: string;
} & AccountRequestQueryParams;

/**query params to fetch a post */
export type PostRequestQueryParams = {
  postId: string;
};

/**query params to fetch a comment */
export type CommentRequestQueryParams = {
  commentId: string;
} & PostRequestQueryParams;

/**query params to fetch a reply */
export type ReplyRequestQueryParams = {
  replyId: string;
} & CommentRequestQueryParams;

export type AudioRequestQueryParams = {
  audioId: string;
};

export type HashTagAndLocationRequestQueryParams = {
  name: string;
};
...
export type AccountPageResponseBodyParams<
  T extends {} = never,
  U extends PageRequestBodyParams = PageRequestBodyParams,
  V extends {} = never
> = ResponseBodyParams<AccountPagePayload, T, U, V>;
...
export type CollectionRequestQueryParams = {
  collectionName: string;
} & PostRequestQueryParams;

export type ReportRequestBodyParams = {
  reason: string;
  category: string;
  description?: string;
};
